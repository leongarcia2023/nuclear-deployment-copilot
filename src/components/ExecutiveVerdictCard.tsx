    ...
    <section className="border border-[#d9d3c8] bg-[#fbfaf7] p-6 sm:p-8">
      <div className="grid gap-6 lg:grid-cols-[1fr_0.34fr]">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-xl leading-8 text-[#3f3d38]">{memo.oneLineJudgment}</p>
        </div>
        <div className="border border-[#d9d3c8] bg-white p-5">
          <p className="text-sm font-semibold text-[#63615b]">Confidence</p>
          <p className="mt-2 text-2xl font-semibold">{memo.confidence}</p>
          <p className="mt-3 text-base leading-7 text-[#4a4842]">First-pass diligence view, not an investment recommendation.</p>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-7 lg:grid-cols-[1fr_0.9fr]">
        <div>
          ...
          <p className="mt-3 text-lg leading-8 text-[#3f3d38]">{memo.recommendedNextAction}</p>
          <button className="mt-5 inline-flex items-center justify-center gap-2 bg-[#151514] px-5 py-3 text-base font-semibold text-white" onClick={handleCopy}>
        ...
        <div>
          ...
          <ol className="mt-3 space-y-2">
            {topKillRisks.map((risk, index) => (
              <li key={risk} className="grid grid-cols-[2rem_1fr] gap-3 text-base leading-7 text-[#3f3d38]">
